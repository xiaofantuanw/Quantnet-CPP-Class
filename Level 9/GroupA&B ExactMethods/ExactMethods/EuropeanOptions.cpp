/*EuropeanOptions.hpp
* Author: Tengxiao Fan
* This is a source file for the EuropeanOptions class. The class describes the characteristics of European options
*/
...
//This function calculates the call option price of a vector of S
//Start from this->S, mesh size is h, length of vector is N
{
...
//This function calculates the put option price of a vector of S
//Start from this->S, mesh size is h, length of vector is N
{
	std::vector<double> result(N);
	for (int i = 0; i < N;i++)
...
//This function calculates the option price of a matrix of parameters
//The parameter with index on index_change will change, other parameters will remain unchanged.
{
	...
	for (int i = 0;i < N;i++)
	{
		...
		temp[index_change] = begin + i * h;//Change the index
		...
		result.push_back(temp);//Push the vector into the matrix
	}
	return result;
}
...
//This function calculates the Delta value of call option
{
	using namespace boost::math;
	normal_distribution<> StdNormal; // Standard normal
	double tmp = m_sig * sqrt(m_T);
...
//This function calculates the Delta value of put option
{
	using namespace boost::math;
	normal_distribution<> StdNormal; // Standard normal
	double tmp = m_sig * sqrt(m_T);
...
//This function calculates the Gamma value of call option
{
	using namespace boost::math;
	normal_distribution<> StdNormal; // Standard normal
	double tmp = m_sig * sqrt(m_T);
...
//This function calculates the Gamma value of put option
{
	using namespace boost::math;
	normal_distribution<> StdNormal; // Standard normal
	double tmp = m_sig * sqrt(m_T);
...
//This function calculates the call option delta of a vector of S
//Start from this->S, mesh size is h, length of vector is N
{
	std::vector<double> result(N);
	for (int i = 0; i < N;i++)
	{
		double S = m_S + i * h;
		result[i] = EuropeanOptions(m_T, m_K, m_sig, m_r, S, m_b).DeltaCall();//Calculate the Delta
	}
	return result;
}
...
//Start from this->S, mesh size is h, length of vector is N
{
	...
	{
		double S = m_S + i * h;
		result[i] = EuropeanOptions(m_T, m_K, m_sig, m_r, S, m_b).DeltaPut();//Calculate the delta
	}
	return result;
}

std::vector<std::vector<double>> EuropeanOptions::SensitiveMat(int index_change, double h, int N) const
//This function calculates the option delta and gamma value of a matrix of parameters
//The parameter with index on index_change will change, other parameters will remain unchanged.
{
	std::vector<std::vector<double>> result;
	std::vector<double> sample{ m_T,m_K,m_sig,m_r,m_S,m_b };
	...
	{
		std::vector<double> temp{ m_T,m_K,m_sig,m_r,m_S,m_b };
		temp[index_change] = begin + i * h;//Change the value of the given index
		...
		result.push_back(temp);//Push the vector to the matrix
	}
	return result;
}

double EuropeanOptions::difDeltaCall(double h) const
//Calculate the delta value of call option using divided difference method
{
...
//Calculate the delta value of put option using divided difference method
{
...
//Calculate the gamma value of call option using divided difference method
{
	double dif= EuropeanOptions(m_T, m_K, m_sig, m_r, m_S + h, m_b).CallPricing() + EuropeanOptions(m_T, m_K, m_sig, m_r, m_S - h, m_b).CallPricing()
...
//Calculate the gamma value of put option using divided difference method
{
	double dif = EuropeanOptions(m_T, m_K, m_sig, m_r, m_S + h, m_b).PutPricing() + EuropeanOptions(m_T, m_K, m_sig, m_r, m_S - h, m_b).PutPricing()
