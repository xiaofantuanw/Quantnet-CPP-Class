...
Line::Line():Shape(),startpoint(0,0), endpoint(0,0)//Default constructor using colon syntax
{
	//startpoint = Point(0, 0);//Set the start point to be (0,0)
	//endpoint = Point(0, 0);//Set the end point to be (0,0)
}

Line::Line(const Point& startpt, const Point& endpt):Shape(),startpoint(startpt),endpoint(endpt)
...
Line::Line(const Line& source) :Shape(source),startpoint(source.startpoint), endpoint(source.endpoint)
...
std::string Line::ToString() const//Make a string that describe the Line
{
	...
	stream <<s<< "; Line: (" << startpoint.X() << ", " << startpoint.Y() << ") to (" << endpoint.X() << ", " << endpoint.Y() << ")";//Make the stream
...
void Line::Draw() const//Draw the shape (simulating with printing)
{
